          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="flex items-center justify-between pt-1">
              ...
              <button
                ...
                className="text-[11px] text-slate-500 hover:text-blue-600 flex items-center gap-1.5 font-medium cursor-pointer"
              >
                <Database className={`w-3 h-3 ${dbConfig.isConnected ? 'text-emerald-500' : 'text-blue-500'}`} />
                <span className={dbConfig.isConnected ? 'text-emerald-700 font-semibold' : ''}>
                  {dbConfig.isConnected ? 'Supabase Connected' : 'Setup Database'}
                </span>
              </button>
            </div>
      ...
      {isDbModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto animate-in fade-in">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-2xl w-full overflow-hidden my-6">
            ...
            {/* ----------------------------------------------------------- */}
            {/* STEP 1: CONNECT DATABASE & AUTO-CONFIGURE                    */}
            {/* ----------------------------------------------------------- */}
            {dbWizardStep === 'CONNECT_DB' && (
